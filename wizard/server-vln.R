...
filterCellsReactive <-
  eventReactive(input$filterCells,{
    withProgress(message = "Processing , please wait",{
      ...
      lowThresh = lapply(seq(length(c(input$subsetNames, "nUMI"))), function(i){
        input[[paste0("thresh",i)]][1]
      })


      highThresh = lapply(seq(length(c(input$subsetNames, "nUMI"))), function(i){
      ...
      pbmc <- FilterCells(object = pbmc, subset.names = c(input$subsetNames, "nUMI"),
      ...
      #logs$VlnPlot <- logs$VlnPlot + 1
      
      #cat(logs$VlnPlot, file="logs\\VlnPlot.txt", append=FALSE)
